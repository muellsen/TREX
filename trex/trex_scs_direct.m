function [betaTREX, betaTREXMat, funTREXMat,out] = trex_scs_direct(Xmat, Yvec, inopts)
...
% (single-thread mode to solve 2p problems)
...
defopts.abstol = 1e-6;  % eps parameter is solution tolerance in SCS solver
defopts.maxit = 5000;
defopts.cpath = 0.5;
...
% Create storage for 2p beta vectors, function values and timings
betaTREXMat = zeros(p, 2 * p,length(cpath));
funTREXMat = zeros(2 * p, length(cpath));
runTimes = zeros(2 * p, length(cpath));
i = 1;
for j = 1:p
    ...
    for s = [-1, 1]
        for k=1:length(cpath)
            c=cpath(k);
            v = s * c * Xmat(:, j);
            data.A(n+2*p + 1, (2 * p + 2):end) = -v;
            data.A(n+2*p + 2, (2 * p + 2):end) = v;
            if k>1
                data.x=xx;
                data.y=yy;
                data.s=ss;
            ...
            tic
            % 2nd argument give dual yy, and the third gives ss
            [xx,yy,ss,info]=scs_direct(data,K,params);
            runTimes(i,k) = toc;
            betaTREXMat(:, i,k) = xx((p + 2):(2 * p + 1));
            funTREXMat(i,k) = info.pobj;
        end
        i = i + 1;
    end        
end
