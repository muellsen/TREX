function [betaTREX,betaTREXMat,funTREXMat,betaBoot,bestBootMat,betaBootMat,funBootMat,out] = trex(Xmat,Yvec,inopts)
...
% Output: TREX solutions: betaTREX (best),betaTREXMat (p x rep solutions in order of
%         beta0 start vectors),funTREXMat (rep corresponding function value)
...
% Output results
% Length of the c regularization path
if nC == 0
    betaTREXMat = zeros(p,numRep);
    funTREXMat = zeros(numRep,1);
    funEvalsTREXMat = zeros(numRep,1);
else
...
for i=1:numRep
    ...
    %tic;
    %[temp1,temp2] = L1General2_PSSgb(@objTREX,xstart,lambdaVec,l1g2opts);
    %pssToc = toc;
    if nC==0
        [betaTREXMat(:,i),funTREXMat(i),funEvalsTREXMat(i)] = L1General2_PSSgb(@objTREX,xstart,lambdaVec,l1g2opts);
    else

        for k=1:nC
            normConst = cpath(k);
            
            [tempBeta,tempFun,tempEvals] = L1General2_PSSgb(@objTREX,xstart,lambdaVec,l1g2opts);

            % warm start
            xstart = tempBeta;

            betaTREXMat(:,i,k) = tempBeta;
            funTREXMat(i,k) = tempFun;
            funEvalsTREXMat(i,k) = tempEvals;
        end
    end
    
    % PNOPT solver (slower than PSS)
%     L1pen = prox_l1(ones(p,1));
%     pnopt_options = pnopt_optimset( 'optim', 1e-8 );
%     pnopt_options.display=-1;
%     pnopt_options = pnopt_optimset( 'max_iter', opts.MaxIter );
%     pnopt_options = pnopt_optimset( 'max_fun_evals', opts.MaxIter*10 );
%     tic
%     [temp3 temp4] = pnopt(@objTREX, L1pen, xstart,pnopt_options);
%     pnoptToc = toc
%     temp2
%     temp4
%     temp4-temp2
%     pause
   ...
   % BFGS solver (slower than PSS)
    %tic
    %[temp3 temp4] = owlbfgs(fun, xx, A, ytr, lambda, varargin)
    %[temp3,result_owlqn]=owlbfgs(@objTREX, xstart, [], [], lambdaVec(1),'display',0,'maxiter',opts.MaxIter,'ftol',opts.optTol,'max_linesearch', 50,'epsg', opts.progTol);
                      
    %owlToc = toc;
    %result_owlqn;
    %temp1'
    %temp3'
    %diffTemp=sum(abs(temp3-temp1))
