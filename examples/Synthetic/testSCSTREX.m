...
% cTREX (SCS) and qTREX(PSS) versions
...
for k = 1:numKappa
    ...
    for s = 1:numSig
        ...
        for r = 1:numRep
            ...
            %tic
            %[betaQ,betaMatQ,funVecQ] = trexp(X,Y,trexopts);
            %solveQTREX = toc;
            ...
            trexSCSopts.c = cVal;        % Constant c
            trexSCSopts.verbose = 0;    % No diagnostic output
            trexSCSopts.maxit = 5000;   % Number of iterations in SCS solver
            trexSCSopts.abstol = 1e-3;  % Solution accuracy in SCS solver
            
            % Single thread version
            [betaC,betaMatC,funVecC,outSCS] = trex_scs(X,Y,trexSCSopts);
            solveCTREX = sum(outSCS.runTimes(:));
            
            % Multi-thread version
            %tic
            %[betaC,betaMatC,funVecC,outSCS] = trex_scsp(X,Y,trexSCSopts);
            %solveCTREX = toc;
            
            % beta vector entries are only approximately zero due to the solver; set small values to 0
            epsSCS = 1e-2;
            betaC(abs(betaC)<epsSCS) = 0;
            betaMatC(betaMatC(:)<epsSCS) = 0;
            ...
            disp(['Function values qTREX, cTREX: ',num2str(min(funVecQ(:))),' ',num2str(min(funVecC(:)))]);
...
save(['ICMLExampleSCS_',date,'_p',num2str(p),'_n',num2str(n),'_nRep',num2str(numRep)])
